...
import org.eenie.wgj.ui.picker.core.WheelScroller;
...
class CrossVerImpl implements ICrossOrientation {
  ...
  @Override
  public void fling(WheelScroller scroller, VelocityTracker tracker, int from, int disMin,
                    int disMax, int over) {
    scroller.fling(0, from, 0, (int) tracker.getYVelocity(), 0, 0, disMin, disMax, 0, over);
  }

  @Override
  public int computeStraightUnit(int unit, int index, int totalMoveX, int totalMoveY,
                                 int singleMoveX, int singleMoveY) {
  ...
  @Override
  public void computeCurItemRect(Rect rect, int space, int width, int height, int textWidth,
                                 int textHeight, int x, int y, int left, int top, int right, int bottom) {
  ...
  @Override
  public void computeRectPadding(Rect rectLast, Rect rectNext, Rect rect, int left, int top,
