...
interface ICrossOrientation {
  ...
  void fling(WheelScroller scroller, VelocityTracker tracker, int from, int disMin, int disMax,
             int over);

  int computeStraightUnit(int unit, int index, int totalMoveX, int totalMoveY, int singleMoveX,
                          int singleMoveY);

  int getUnitDeltaTotal(int totalMoveX, int totalMoveY);

  void computeCurItemRect(Rect rect, int space, int width, int height, int textWidth,
  ...
  void computeRectPadding(Rect rectLast, Rect rectNext, Rect rect, int left, int top, int right,
                          int bottom);
